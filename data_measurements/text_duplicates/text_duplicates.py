...
class DMTHelper:
    ...
    def __init__(self, dstats, load_only, save):
        ...
        self.dups_result_json_fid = pjoin(self.cache_dir, self.dups_dir,
                                          dups_json)
        self.dups_result_html_fid = pjoin(self.cache_dir, self.dups_dir,
                                          dups_html)
    ...
    def _prepare_duplicates(self, list_duplicates=True):
        ...
        results = duplicates.compute(data=self.dset,
                                     list_duplicates=list_duplicates)
        return results
    ...
    def _write_duplicates_cache(self):
        ...
        if self.duplicates_results:
            ds_utils.write_json(self.duplicates_results,
                                self.dups_result_json_fid)
            ...
            ds_utils.write_json_as_html(self.duplicates_results,
                                        self.dups_result_html_fid)

    def get_duplicates_filenames(self):
        dups_fid_dict = {"statistics": self.dups_result_json_fid,
                         "html": self.dups_result_html_fid}
