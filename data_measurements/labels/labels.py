...
def make_label_results_dict(label_measurement, label_names):
    label_dict = {LABEL_MEASUREMENT: label_measurement, LABEL_NAMES: label_names}
    return label_dict
...
def make_label_fig(label_results, chart_type="pie"):
    try:
        ...
        if chart_type == "bar":
            fig_labels = plt.bar(label_measurement[EVAL_LABEL_MEASURE][EVAL_LABEL_ID],
        ...
        print("Input label data missing required key(s).")
        print("We require %s, %s, %s" % (LABEL_NAMES, LABEL_MEASUREMENT))
        print("We found: %s" % ",".join(label_results.keys()))
        return False
    return fig_labels
