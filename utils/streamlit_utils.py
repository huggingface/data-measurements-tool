...
def expander_zipf(dstats, column_id=""):
    ...
    with st.expander(
            f"Vocabulary Distribution{column_id}: Zipf's Law Fit",
            expanded=False
    ):
        try:
            _ZIPF_CAPTION = """This shows how close the observed language is 
            to an ideal natural language distribution following 
            [Zipf's law](https://en.wikipedia.org/wiki/Zipf%27s_law),
            calculated by minimizing the 
            [Kolmogorov-Smirnov (KS) statistic](https://en.wikipedia.org/wiki/Kolmogorov%E2%80%93Smirnov_test)."""
            ...
            zipf_summary += (
                    ...
                    + str(int(z.xmin))
                    + "**, which is the optimal rank *beyond which* the scaling"
                      " regime of the power law fits best."
            )

            alpha_warning = "Your alpha value is a bit on the high side, which" \
                            " means that the distribution over words in this" \
                            " dataset is a bit unnatural. This could be due to" \
                            " non-language items throughout the dataset."
            xmin_warning = "The minimum rank for this fit is a bit on the high" \
                           " side, which means that the frequencies of your" \
                           " most common words aren't distributed as would be" \
                           " expected by Zipf's law."
            ...
            st.caption(
                "Use this widget for the counts of different words in your "
                "dataset, measuring the difference between the observed count "
                "and the expected count under Zipf's law."
            )
            st.markdown(_ZIPF_CAPTION)
            st.write("""A Zipfian distribution follows the power law: 
            $p(x) \propto x^{-α}$
            with an ideal α value of 1.""")
            st.markdown(
                "In general, an alpha greater than 2 or a minimum rank greater"
                " than 10 (take with a grain of salt) means that your"
                " distribution is relativaly _unnatural_ for natural language."
                " This can be a sign of mixed artefacts in the dataset,"
                " such as HTML markup."
            )
            st.markdown(
                "Below, you can see the counts of each word in your dataset vs."
                " the expected number of counts following a Zipfian distribution."
            )
            st.markdown("-----")
            ...
            if z.ks_test.pvalue < 0.01:
                st.markdown(
                    "\n Great news! Your data fits a powerlaw with a minimum KS"
                    "distance of %.4f" % z.distance)
            else:
            ...
            logs.warning("Hit zipf error %s " % e)
